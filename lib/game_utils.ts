...
export async function generateFactQuiz(count = 20) {
  ...
  return selection.map((p: any, i: number) => {
...
export async function generateScrambleQuiz(count = 10) {
  const selection = await Pokemon.aggregate([
    { $sample: { size: count } },
    { $project: { name: 1 } },
  ]);
...
export async function generateImageQuiz(count = 10) {
  const selection = await Pokemon.aggregate([
  ...
  return selection.map((p: any, i: number) => {
    const incorrect = distractorPool
      .slice(i * 3, i * 3 + 3)
      .map((d: any) => d.name)
      .filter((name: string) => name !== p.name);
